...
class ContentGeneratorAgent:
    ...
    def _build_content_bundle(self, product, position, strategy_signal=None):
        ...
        # Instagram için görsel URL oluştur - HER ZAMAN dolu olmalı
        # Öncelik: product_url'den görsel çıkarmaya çalış
        # Fallback: Placeholder görsel
        image_url = None
        
        # Eğer product_url varsa ve bilinen e-ticaret sitelerinden ise
        if product_url:
            product_url_lower = product_url.lower()
            if "trendyol" in product_url_lower:
                # Trendyol ürün görseli URL'si çıkarma (basit örnek)
                # Gerçek uygulamada Trendyol API kullanılmalı
                image_url = f"https://picsum.photos/800/800?text={product_name[:20].replace(' ', '+')}"
            elif "hepsiburada" in product_url_lower:
                # Hepsiburada ürün görseli URL'si çıkarma
                image_url = f"https://picsum.photos/800/800?text={product_name[:20].replace(' ', '+')}"
            elif "n11" in product_url_lower:
                image_url = f"https://picsum.photos/800/800?text={product_name[:20].replace(' ', '+')}"
        
        # Eğer hala image_url yoksa, her zaman bir placeholder oluştur
        if not image_url:
            # Rastgele seed ile her seferinde farklı görsel
            import random
            seed = random.randint(1000, 9999)
            image_url = f"https://picsum.photos/800/800?random={seed}&text={product_name[:15].replace(' ', '+')}"
        
        # image_url'ın mutlaka dolu olduğunu garanti et
        if not image_url or not isinstance(image_url, str):
            image_url = f"https://picsum.photos/800/800?random={int(datetime.now().timestamp())}"
        ...
        return {
            ...
            "image_url": image_url,  # Instagram için görsel URL - HER ZAMAN dolu
            # Çoklu anahtar desteği - farklı sistemlerle uyumluluk için
            "image": image_url,
            "photo": image_url,
            "picture": image_url,
            "media_url": image_url,
