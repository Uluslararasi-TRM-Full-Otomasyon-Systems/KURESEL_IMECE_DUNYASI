...
async def cmd_full():
    """Tüm sistemleri başlat - MASTER_CONTROLLER ve Orchestrator API"""
    import subprocess
    import sys
    ...
    # MASTER_CONTROLLER'ı başlat (arka plan)
    print("📡 MASTER_CONTROLLER başlatılıyor...")
    try:
        import os
        import sys
        ...
        # Controller'ı thread'de çalıştır
        import threading
        controller_thread = threading.Thread(target=lambda: controller.start_24_7_mode(), daemon=True)
        controller_thread.start()
        print("✅ MASTER_CONTROLLER başlatıldı")
    except Exception as e:
        print(f"⚠️ MASTER_CONTROLLER başlatılamadı: {e}")
    
    # Orchestrator API'yi başlat
    print("🌐 Orchestrator API başlatılıyor...")
    try:
        sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        from orchestrator_api import app
        import uvicorn
        
        # Bulut ortamında 0.0.0.0 dinle
        config = uvicorn.Config(app, host="0.0.0.0", port=int(os.getenv("PORT", 5000)))
        server = uvicorn.Server(config)
        await server.serve()
    except Exception as e:
        print(f"⚠️ Orchestrator API başlatılamadı: {e}")
        # Fallback: basit bir HTTP server
        print("🔄 Fallback HTTP server başlatılıyor...")
        from http.server import HTTPServer, SimpleHTTPRequestHandler
        server = HTTPServer(("0.0.0.0", int(os.getenv("PORT", 5000))), SimpleHTTPRequestHandler)
