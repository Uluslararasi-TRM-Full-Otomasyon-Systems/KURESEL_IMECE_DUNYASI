...
# -*- coding: utf-8 -*-
"""
TRM Nirvana v3.0 - Satis Alarm ve Uyari Sistemi
Panel uzerinden satis hareketlerini takip eder
"""
...
class SalesAlarmSystem:
    ...
    async def track_sales_activity(self):
        ...
        try:
            ...
            logger.error(f"Satis takibi hatasi: {e}")
            return None
    ...
    async def check_alerts(self, sales_activity: Dict):
        ...
        # Yuksek komisyonlu urun alarmi
        if sales_activity.get('high_commission', 0) >= 3:
            alerts.append({
                'type': 'high_commission',
                'level': 'success',
                'title': '🔥 Yuksek Komisyonlu Urunler!',
                'message': f"Bugun {sales_activity['high_commission']} adet %20+ komisyonlu urun yakalandi!",
                'timestamp': datetime.now().isoformat(),
                'data': sales_activity
            })
        
        # Basarili paylasim alarmi
        if sales_activity.get('social_published', 0) >= 5:
            alerts.append({
                'type': 'social_success',
                'level': 'success',
                'title': '📱 Sosyal Medya Basarisi!',
                'message': f"Bugun {sales_activity['social_published']} adet sosyal medya paylasimi yapildi!",
                'timestamp': datetime.now().isoformat(),
                'data': sales_activity
            })
        
        # Tahmini komisyon alarmi
        if sales_activity.get('estimated_commission', 0) >= 50:
            alerts.append({
                'type': 'commission_alert',
                'level': 'warning',
                'title': '💰 Komisyon Hedefi!',
                'message': f"Tahmini gunluk komisyon: {sales_activity['estimated_commission']:.2f} TL",
                'timestamp': datetime.now().isoformat(),
                'data': sales_activity
            })
        ...
        if sales_activity.get('success_rate', 0) >= 80:
            alerts.append({
                'type': 'success_rate',
                'level': 'info',
                'title': '📊 Yuksek Basari Orani!',
                'message': f"Sistem basari orani: {sales_activity['success_rate']:.1f}%",
                'timestamp': datetime.now().isoformat(),
                'data': sales_activity
            })
    ...
    async def send_alert(self, alert: Dict):
        ...
        try:
            ...
            logger.info(f"Alert gonderildi: {alert['title']}")
            
        except Exception as e:
            logger.error(f"Alert gonderme hatasi: {e}")
    
    async def add_panel_alert(self, alert: Dict):
        """Panel'e alert ekle"""
        try:
            # Alert'i JSON dosyasina kaydet
            ...
            logger.error(f"Panel alert ekleme hatasi: {e}")
    
    async def send_telegram_alert(self, alert: Dict):
        """Telegram alert'i gonder"""
        try:
...
📊 Detaylar:
• Yakalanan Urun: {alert['data'].get('products_captured', 0)}
• %20+ Urun: {alert['data'].get('high_commission', 0)}
• Sosyal Paylasim: {alert['data'].get('social_published', 0)}
• Tahmini Komisyon: {alert['data'].get('estimated_commission', 0):.2f} TL
• Basari Orani: {alert['data'].get('success_rate', 0):.1f}%

⏰ {alert['timestamp']}
            """
            
            # Telegram gonderimi (mock)
            logger.info(f"Telegram alert: {message}")
            
        except Exception as e:
            logger.error(f"Telegram alert gonderme hatasi: {e}")
    ...
    def get_sales_summary(self) -> Dict:
        ...
        if not recent_sales:
            recent_sales = self.sales_data[-10:]  # Son 10 kayit
...
class SalesAlertAPI:
    ...
    async def start_monitoring(self):
        """Izlemeyi baslat"""
        logger.info("🚨 Satis alarm sistemi baslatiliyor...")
        ...
        while True:
            try:
                ...
                logger.error(f"Satis izleme hatasi: {e}")
...
async def test_sales_alarm():
    """Satis alarm sistemini test et"""
    alarm_system = SalesAlarmSystem()
    
    logger.info("🚨 Satis alarm sistemi test ediliyor...")
