...
class InstagramPublisher:
    ...
    async def publish_content(self, content: Dict) -> Dict:
        ...
        async def _post():
            ...
            # Çoklu anahtar desteği - image_url asla boş olmamalı
            image_url = (
                content.get('image_url') or 
                content.get('image') or 
                content.get('photo') or 
                content.get('picture') or 
                content.get('media_url') or 
                content.get('media') or 
                content.get('img') or
                content.get('url') or
                ''
            )
            
            # Son güvenlik katmanı - hala boşsa placeholder
            if not image_url:
                image_url = f"https://picsum.photos/800/800?random={int(datetime.now().timestamp())}"
                logger.warning("Instagram için image_url bulunamadı, placeholder kullanılıyor")
...
class SocialMediaManager:
    ...
    async def publish_to_all_platforms(self, content: Dict) -> Dict:
        ...
        logger.info("📡 Tum platformlarda yayinlama baslatiliyor...")
        
        # Nirvana Shield: Fingerprint maskeleme
        user_id = content.get('user_id', 'default_user')
        fingerprint = self.shield.get_masked_fingerprint(user_id)
        logger.info(f"🔒 Maskelenmiş parmak izi: {fingerprint}")
        
        # Nirvana Shield: Chaos jitter ile paylaşım aralığı
        jitter_wait = self.shield.apply_chaos_jitter(base_seconds=3)
        logger.info(f"⏱️ Chaos jitter bekleme süresi: {jitter_wait:.2f}s")
    ...
    async def publish_single(self, platform: str, content: Dict) -> Dict:
        ...
        # Nirvana Shield: Chaos jitter ile paylaşım aralığı
        jitter_wait = self.shield.apply_chaos_jitter(base_seconds=2)
        logger.info(f"⏱️ Chaos jitter bekleme süresi: {jitter_wait:.2f}s")
